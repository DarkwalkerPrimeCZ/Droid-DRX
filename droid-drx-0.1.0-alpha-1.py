...
def check_ntp_availability():
    for server in NTP_SERVERS:
        if get_ntp_time(server) is not None:
            return True
    return False
...
class Blockchain:
        ...
        try:
            while self.mining_in_progress:
                if not p2p_node.get_online_peers():
                    sys.stdout.write(f"\r{Fore.YELLOW}Tìžba zastavena: Ztraceno spojení se všemi uzly.{Style.RESET_ALL}\n")
                    sys.stdout.flush()
                    stop_event.set()
                    self.mining_in_progress = False
                    stop_mining = True
                ...
                if not check_ntp_availability():
                    sys.stdout.write(f"\r{Fore.YELLOW}Tìžba zastavena: Žádný NTP server není dostupný.{Style.RESET_ALL}\n")
                    sys.stdout.flush()
                    stop_event.set()
                    self.mining_in_progress = False
                    stop_mining = True
    ...
    def mine(self, miner_address):
        ...
        while True:
            ...
            proof, stop_mining = self.proof_of_work(new_block, num_cores)
            if stop_mining:
                return False
            elif proof is None:
                continue
...
def print_menu():
    ...
    print(f"{Fore.GREEN}12{Style.RESET_ALL} - Ukonèit a uložit")
